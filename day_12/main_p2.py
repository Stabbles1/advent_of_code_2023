...
def num_of_valid_combinations(layout: str, broken:list[int]) -> int:
    ...
    print(f"start={2**number_of_brokens_to_place} end={2**num_of_unknown_locations} diff={(2**num_of_unknown_locations) - (2**number_of_brokens_to_place)}")
    start = 2**number_of_brokens_to_place - 1
    end = 2**num_of_unknown_locations
    binint = start
    while binint <= end:
        combo = format(binint, formatter)
        #print(combo)
        if combo.count("1") != number_of_brokens_to_place:
            #print(f"Not enough {combo.count('1')} is less than {number_of_brokens_to_place}")
            binint += 1
            continue
        #print(f"ENOUGH @ {binint}")
        result = is_valid(combo, layout, broken_str)
        if result != False:
